...
from infra.deployment.remote.ops import RailwayRemoteOpsProvider, RemoteOpsError, RemoteServiceScope
...
def test_fetch_logs_returns_captured_output() -> None:
    ...
    with (
        ...
        patch(
            "infra.deployment.remote.ops.subprocess.run", side_effect=_make_run(log_output=log_text)
        ),
    ):
...
def test_fetch_logs_returns_empty_string_when_no_output() -> None:
    ...
    with (
        patch("infra.deployment.remote.ops.shutil.which", return_value="/usr/local/bin/railway"),
        patch("infra.deployment.remote.ops.subprocess.run", side_effect=_make_run(log_output="")),
    ):
        result = provider.fetch_logs(scope, lines=10)
